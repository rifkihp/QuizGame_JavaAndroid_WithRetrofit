...
import com.example.quizgame.retrofit.RestApi;
import com.example.quizgame.retrofit.RetroFit;
...
public class QuizActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Call<ResponseRequestSoal> soalCall = api.getSoalQuiz();
        soalCall.enqueue(new Callback<ResponseRequestSoal>() {

            @Override
            public void onResponse(@NonNull Call<ResponseRequestSoal> call, @NonNull Response<ResponseRequestSoal> response) {

                if(getSelectedTopicName.equals("java")) {
                    questionsList = Objects.requireNonNull(response.body()).getSoalJava();
                } else
                if(getSelectedTopicName.equals("php")) {
                    questionsList = Objects.requireNonNull(response.body()).getSoalPhp();
                } else
                if(getSelectedTopicName.equals("python")) {
                    questionsList = Objects.requireNonNull(response.body()).getSoalPython();
                } else {
                    questionsList = Objects.requireNonNull(response.body()).getSoalJavascript();
                }

                //ACAK SOAL
                for (int i = 0; i < 13; i++) {
                    int rando = (int) ((Math.random() * questionsList.size()));
                    questionsList.remove(rando);
                }
            ...
            @Override
            public void onFailure(@NonNull Call<ResponseRequestSoal> call, @NonNull Throwable t) {
